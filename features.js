import {partyOf,partySummary,canFit} from './party.js?v=20260926-members1';
import {today, addDays, nights, validStay, available, rate} from './model.js?v=20260926-members1';
...
export function createFeatures(ctx) {
  ...
  const guestTypes = () => getState().types.filter(t => canFit(t,getSearch()));
  ...
  function showCompare(initialize = true) {
    ...
    const rows = [
      ...
      ['ราคา / ห้อง / คืน', t=>`฿${money(rate(t))}`],
      [`รวม ${partyOf(s).rooms} ห้อง × ${nights(s.start,s.end)} คืน`, t=>`<strong>฿${money(rate(t)*nights(s.start,s.end)*partyOf(s).rooms)}</strong>`],
    ...
    wideModal(header('เปรียบเทียบห้องพัก', `${fmt(s.start)} — ${fmt(s.end)} · ${partySummary(s)} · ข้อมูลและราคาสมมติ`)+`<div class="modal-body compare-body"><div class="compare-picker" role="group" aria-label="เลือกประเภทห้องเปรียบเทียบ">${getState().types.map(t=>`<button class="${selected.has(t.id)?'active':''}" data-action="feature-compare-pick" data-id="${t.id}" aria-pressed="${selected.has(t.id)}">${selected.has(t.id)?'✓ ':''}${t.name}</button>`).join('')}</div><p class="feature-muted">เลือก 2–4 ประเภทเพื่อเทียบในช่วงวันเดียวกัน</p>${types.length>=2?`<div class="compare-scroll" tabindex="0" aria-label="ตารางเปรียบเทียบ เลื่อนแนวนอนเพื่อดูทุกประเภท"><table class="compare-table"><thead><tr><th scope="col">รายละเอียด</th>${types.map(t=>`<th scope="col"><img src="${esc(t.photo)}" alt="ภาพสมมติ ${t.name}"><span>${t.name}</span></th>`).join('')}</tr></thead><tbody>${rows.map(([label,value])=>`<tr><th scope="row">${label}</th>${types.map(t=>`<td>${value(t)}</td>`).join('')}</tr>`).join('')}<tr><th scope="row">เลือกห้อง</th>${types.map(t=>{const can=canFit(t,s)&&available(getState(),t.id,s.start,s.end).length>=partyOf(s).rooms;return `<td><button class="primary" data-action="book" data-id="${t.id}" ${can?'':'disabled'}>${!canFit(t,s)?'จำนวนผู้เข้าพักเกิน':can?'จองห้องนี้':'เต็มในช่วงนี้'}</button></td>`}).join('')}</tr></tbody></table></div>`:'<div class="empty">กรุณาเลือกอีกอย่างน้อย 1 ประเภท</div>'}<p class="feature-muted">รายละเอียดจริงจะเปลี่ยนตามข้อมูลที่โรงแรมอนุมัติ · ตารางนี้เปรียบเทียบห้องของโรงแรมเอง</p></div>`);
  }
  ...
  function handleAction(action, id) {
    ...
    switch(action) {
      ...
      case 'feature-day': {
        ...
        else {if(available(getState(),calendarType,id,addDays(id,1)).length<partyOf(getSearch()).rooms){toast('คืนนี้มีห้องว่างไม่พอตามจำนวนที่เลือก กรุณาเลือกวันเข้าอื่น');break;}draft={start:id,end:''};}
      ...
      case 'feature-use-dates': {
        ...
        if(!canFit(byId(calendarType),getSearch())||available(getState(),calendarType,draft.start,draft.end).length<partyOf(getSearch()).rooms) throw Error('ห้องไม่ตรงกับช่วงวันหรือจำนวนผู้เข้าพัก');
